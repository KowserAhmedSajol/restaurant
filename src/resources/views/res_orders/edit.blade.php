<x-smart-master>
    <x-smart-card>
        ...
        <x-slot name="body">
            ...
            <form action="{{ route('res_orders.update', $res_order->uuid) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <!-- Dynamic Fields -->
    ...
    <div class="{{$decoration['class']['elementcontainer'] }}">
        ...
        <x-smart-selecttrio tableName="res_categorys" name="res_category" :value="old('res_category', $res_order->res_category)"/>
    </div>
</div><div class="{{$decoration['class']['elementwrapper']}}">
    <div class="{{$decoration['class']['elementcontainer'] }}">
        <x-smart-label for="res_productInput">{{ __('Res_product') }}</x-smart-label>
        <x-smart-selecttrio tableName="res_products" name="res_product" :value="old('res_product', $res_order->res_product)"/>
    </div>
</div><div class="{{$decoration['class']['elementwrapper']}}">
    <div class="{{$decoration['class']['elementcontainer'] }}">
        <x-smart-label for="order_noInput">{{ __('Order_no') }}</x-smart-label>
        <x-smart-text type="text" id="order_noInput" name="order_no" :value="old('order_no', $res_order->order_no)" placeholder="Enter Order_no" />
    </div>
</div><div class="{{$decoration['class']['elementwrapper']}}">
    <div class="{{$decoration['class']['elementcontainer'] }}">
    ...
    @foreach($options as $option)
        <option value="{{ $option }}" {{ old('status', $res_order->status) == $option ? 'selected' : '' }}>{{ $option }}</option>
    @endforeach
</x-smart-select>
    </div>
</div><div class="{{$decoration['class']['elementwrapper']}}">
