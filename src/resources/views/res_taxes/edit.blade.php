<x-smart-master>
    <x-smart-card>
        ...
        <x-slot name="body">
            ...
            <form action="{{ route('res_taxes.update', $res_tax->uuid) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <!-- Dynamic Fields -->
    ...
    <div class="{{$decoration['class']['elementcontainer'] }}">
        ...
        <x-smart-text type="text" id="nameInput" name="name" :value="old('name', $res_tax->name)" placeholder="Enter Name" />
    </div>
</div><div class="{{$decoration['class']['elementwrapper']}}">
    <div class="{{$decoration['class']['elementcontainer'] }}">
        <x-smart-label for="percentageInput">{{ __('Percentage') }}</x-smart-label>
        <x-smart-text type="text" id="percentageInput" name="percentage" :value="old('percentage', $res_tax->percentage)" placeholder="Enter Percentage" />
    </div>
</div><div class="{{$decoration['class']['elementwrapper']}}">
    <div class="{{$decoration['class']['elementcontainer'] }}">
